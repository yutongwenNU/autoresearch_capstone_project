...
def plot_results():
    ...
    for status, color in colors.items():
        ...
        plt.scatter(subset.index + 1, subset['val_rmse'], c=color, label=status, s=100, edgecolors='black')

    # Best-so-far line
    plt.plot(df.index + 1, df['val_rmse'].cummin(), color='#2ecc71', linestyle='--', alpha=0.6, label="Best RMSE")
