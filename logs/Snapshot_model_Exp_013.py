...
# NLP founder-presence regexes. Operate on case-preserved text from
# `Short Description` + `Keywords`. Cases (a, b, c) match the user spec:
#   (a) "founder" within ~50 chars of leadership-role tokens — case-insensitive
#   (b) "Founder and/&/+/CEO" phrasing — case-insensitive
#   (c) Named founder: "founder [Capitalized Name]" or "founded by [Name]" —
#       case-sensitive, since the capital letter is the name signal
FOUNDER_LED_REGEXES = [
    re.compile(r"\bfounder\b.{0,50}\b(?:ceo|president|leadership|cto|coo|cfo)\b", re.IGNORECASE),
    re.compile(r"\b(?:ceo|president|leadership|cto|coo|cfo)\b.{0,50}\bfounder\b", re.IGNORECASE),
    re.compile(r"\bfounder\s*(?:and|&|/)?\s*(?:ceo|president|owner)\b", re.IGNORECASE),
    re.compile(r"\bfounders?\s+[A-Z][a-z]+"),
    re.compile(r"\bfounded\s+(?:in\s+\d{4}\s+)?by\s+[A-Z][a-z]+"),
]


def is_founder_led(text: str) -> int:
    if not text:
        return 0
    for pattern in FOUNDER_LED_REGEXES:
...
def featurize(df: pd.DataFrame, mgmt_depth: pd.Series) -> pd.DataFrame:
    ...
    # Case-preserving text for founder detection (the "founder [Capitalized Name]"
    # and "founded by [Name]" patterns rely on capital letters as a name signal,
    # so we cannot reuse the lowercased `text` bundle above).
    founder_text = (
        df.get("Short Description", pd.Series([""] * len(df))).fillna("").astype(str) + " "
        + df.get("Keywords", pd.Series([""] * len(df))).fillna("").astype(str)
    )
    founder_led = founder_text.apply(is_founder_led)
...
def main():
    ...
    X = X_df.values
    n_founder_led = int(X_df["founder_led"].sum())
    print(f"founder_led tagged: {n_founder_led}/{len(df)} firms")
