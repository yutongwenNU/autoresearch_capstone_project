...
def featurize(df: pd.DataFrame, mgmt_depth: pd.Series) -> pd.DataFrame:
    ...
    stagnant_legacy = tenure * rev_per_emp
    ...
    return pd.DataFrame({
        ...
        "stagnant_legacy": stagnant_legacy,
    })


def build_model() -> GridSearchCV:
    # Autonomous internal tuning (per Week-5 agent-autonomy clause):
    # GridSearchCV over Ridge alpha. The grid is logged in the Research Log.
    base = Pipeline([
...
def main():
    ...
    # Diagnostic: GridSearchCV result + standardized Ridge coefficients at
    # the chosen alpha. Predictions above used nested CV (cross_val_predict
    # over GridSearchCV); this full-data refit is for logging only.
    full_model = build_model().fit(X, y)
    best_alpha = full_model.best_params_["ridge__alpha"]
    print(f"GridSearchCV alpha grid: [0.1, 1.0, 10.0, 100.0]; chosen alpha (full-data inner CV): {best_alpha}")
    print("Per-alpha mean RMSE (5-fold inner CV on full data):")
    for params, mean_score in zip(
        full_model.cv_results_["params"],
        full_model.cv_results_["mean_test_score"],
    ):
        marker = "  <-- chosen" if params["ridge__alpha"] == best_alpha else ""
        print(f"  alpha={params['ridge__alpha']:>6.1f}: RMSE={-mean_score:.4f}{marker}")
    coefs = full_model.best_estimator_.named_steps["ridge"].coef_
    feature_names = list(featurize(df, pd.Series(depths)).columns)
    coef_pairs = sorted(zip(feature_names, coefs), key=lambda x: -abs(x[1]))
    print(f"Ridge coefficients (standardized) at alpha={best_alpha}:")
