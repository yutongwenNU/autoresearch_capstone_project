...
# exp_017 — Stagnation Ratio: numeric (legacy_count + 1) / (modern_count + 1).
# High ratio ⇒ legacy-heavy firm with little modernization talk; low ratio ⇒
# modern-cloud-first firm. Smoothing constants prevent division by zero and
# soften the ratio for sparse counts.
LEGACY_KW = [
    "cabling", "repair", "hardware", "on-premise", "server room",
    "legacy", "phone systems",
]
MODERN_KW = [
    "ai", "cloud-first", "automation", "transformation", "digital-first",
    "next-gen",
]


def stagnation_ratio_score(text: str) -> float:
    t = (text or "").lower()
    legacy_count = sum(1 for k in LEGACY_KW if k in t)
    modern_count = sum(1 for k in MODERN_KW if k in t)
    return (legacy_count + 1) / (modern_count + 1)
...
def featurize(df: pd.DataFrame, mgmt_depth: pd.Series) -> pd.DataFrame:
    ...
    # Stagnation ratio uses the wider text bundle (Rationale + Keywords +
    # Technologies + Short Description) to maximize signal density on a numeric
    # ratio feature.
    stagnation_ratio = text.apply(stagnation_ratio_score)
...
def main():
    ...
    X = X_df.values
    sr = X_df["stagnation_ratio"]
    print(f"stagnation_ratio: min={sr.min():.3f}, mean={sr.mean():.3f}, max={sr.max():.3f}")
