...
# exp_020 — Refined Ownership Red-Flag. High-precision "kill switch" using
# only the explicit acquisition phrases (no growth/inc-5000 keywords that
# fired the Exp_014 false positive on World Synergy).
OWNERSHIP_RED_FLAG_KW = [
    "acquired by", "subsidiary of", "division of",
]
OWNERSHIP_RED_FLAG_REGEXES = [
    re.compile(r"\bpart of the\s+\w+\s+family\b", re.IGNORECASE),
]


def has_ownership_red_flag(text: str) -> int:
    if not text:
        return 0
    t = text.lower()
    for k in OWNERSHIP_RED_FLAG_KW:
        if k in t:
            return 1
    for pat in OWNERSHIP_RED_FLAG_REGEXES:
        if pat.search(text):
            return 1
    return 0
...
def featurize(df: pd.DataFrame, mgmt_depth: pd.Series) -> pd.DataFrame:
    ...
    ownership_red_flag = own_text.apply(has_ownership_red_flag)
    ...
    return pd.DataFrame({
        ...
        "ownership_red_flag": ownership_red_flag,
    })
...
def main():
    ...
    X = X_df.values
    n_orf = int(X_df["ownership_red_flag"].sum())
    print(f"ownership_red_flag tagged: {n_orf}/{len(df)} firms")
