...
### Configuration
* **Worker:** `model.py` — Ridge(alpha=1.0), unchanged from exp_003
...
### Decision: Discard
Although the run was logged with `--keep`, the RMSE regressed by 0.0094 — exp_003 (RMSE 1.5016) remains the current best. **`model.py` will be reverted to the exp_003 feature set before exp_005**, and exp_004 stands as a logged-but-not-merged diagnostic experiment that informed the model-class pivot.
