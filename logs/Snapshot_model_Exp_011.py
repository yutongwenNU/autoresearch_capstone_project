...
def build_model() -> BaggingRegressor:
    base = Pipeline([
...
def main():
    ...
    # Diagnostic: across the 50 bagged estimators, mean ± std of each
    # standardized Ridge coefficient. Std is the variance-reduction signal —
    # large std means there's real per-bag instability that bagging is
    # smoothing; small std means each bag finds nearly the same coefficients
    # and bagging is mostly cosmetic.
    full_model = build_model().fit(X, y)
    all_coefs = np.stack([
        est.named_steps["ridge"].coef_ for est in full_model.estimators_
    ])
    mean_coefs = all_coefs.mean(axis=0)
    std_coefs = all_coefs.std(axis=0)
    feature_names = list(featurize(df, pd.Series(depths)).columns)
    triples = sorted(
        zip(feature_names, mean_coefs, std_coefs), key=lambda x: -abs(x[1])
    )
    print(f"Bagged Ridge coefficients (standardized) — mean ± std across {len(full_model.estimators_)} bags:")
    for name, m, s in triples:
