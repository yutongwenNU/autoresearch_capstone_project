...
PREMIUM_MRR_KW = [
    "managed services", "managed it", "recurring", "mrr",
    "sla", "monthly subscription", "subscription",
]
STANDARD_SERVICE_KW = [
    "help desk", "monitoring", "backup", "compliance", "business continuity",
]
PREMIUM_MRR_WEIGHT = 2.5
STAGNATION_KW = [
...
def featurize(df: pd.DataFrame, mgmt_depth: pd.Series) -> pd.DataFrame:
    ...
    recurring = text.apply(
        lambda t: PREMIUM_MRR_WEIGHT * count_kw(t, PREMIUM_MRR_KW) + count_kw(t, STANDARD_SERVICE_KW)
    )
