...
# Institutionalization "Red Flag" signals — a firm matching any of these in its
# Short Description or Keywords is likely already acquired, professionally
# managed, or institutionally backed, and is therefore a worse search-fund
# target than the structural features alone might suggest.
INSTITUTIONALIZED_KW = [
    # Explicit acquisition / ownership
    "acquired by", "subsidiary of", "division of",
    # Financial / institutional backing
    "private equity", "venture capital", "investment from", "funding",
    # Modernization / growth-press signals
    "rapidly growing", "inc 5000", "award-winning growth",
]
# Regex for "part of the [X] family" — needs a wildcard for the parent name.
INSTITUTIONALIZED_REGEXES = [
    re.compile(r"\bpart of the\s+\w+\s+family\b", re.IGNORECASE),
]


def is_institutionalized(text: str) -> int:
    if not text:
        return 0
    t = text.lower()
    for k in INSTITUTIONALIZED_KW:
        if k in t:
            return 1
    for pat in INSTITUTIONALIZED_REGEXES:
        if pat.search(text):
            return 1
    return 0
...
def featurize(df: pd.DataFrame, mgmt_depth: pd.Series) -> pd.DataFrame:
    ...
    is_inst = inst_text.apply(is_institutionalized)
...
def main():
    ...
    X = X_df.values
    n_inst = int(X_df["is_institutionalized"].sum())
    print(f"is_institutionalized tagged: {n_inst}/{len(df)} firms")
