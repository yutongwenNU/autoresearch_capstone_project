"""
Worker (model.py) — Ridge regression with engineered features
...
# exp_019 — Logistical Hub Proximity. User-supplied hubs: Chicago, Milwaukee,
# St. Louis. Per the autonomy grant, extended with the major Midwest IT
# corridors that share the same trait (dense client population, short
# windshield time): Detroit/SE Michigan, Minneapolis-St. Paul, Columbus,
# Cleveland, Cincinnati, Indianapolis, Kansas City. All entries lowercased.
HUB_CITIES = {
    # Chicago hub (user-listed + nearby suburbs visible in train_set)
    "chicago", "naperville", "elgin", "joliet", "schaumburg", "evanston",
    "glenview", "crystal lake", "oak brook", "rosemont", "aurora",
    "des plaines", "woodstock",
    # Milwaukee hub (user-listed + adjacent)
    "milwaukee", "waukesha", "brookfield", "west allis",
    "wauwatosa", "menomonee falls",
    # St. Louis hub (user-listed; with St./Saint variants)
    "st. louis", "st louis", "saint louis",
    "st. charles", "st charles", "saint charles",
    "chesterfield", "clayton", "creve coeur", "maryland heights",
    "o'fallon",
    # Detroit / SE Michigan corridor (autonomous addition)
    "detroit", "livonia", "troy", "southfield", "ann arbor", "royal oak",
    "farmington hills", "bloomfield hills", "auburn hills", "dearborn",
    "novi",
    # Twin Cities (Minneapolis-St. Paul) corridor
    "minneapolis", "st. paul", "st paul", "saint paul",
    "bloomington", "eden prairie", "edina", "plymouth", "minnetonka",
    "maple grove", "eagan",
    # Columbus, OH corridor
    "columbus", "dublin", "westerville", "hilliard", "worthington", "new albany",
    # Cleveland, OH corridor
    "cleveland", "beachwood", "independence", "mentor", "mayfield",
    "mayfield heights", "solon", "westlake",
    # Cincinnati, OH corridor
    "cincinnati", "mason", "blue ash", "west chester",
    # Indianapolis, IN corridor
    "indianapolis", "carmel", "fishers", "greenwood", "plainfield",
    "noblesville",
    # Kansas City (KS/MO border) corridor
    "kansas city", "overland park", "lenexa", "olathe", "leawood", "shawnee",
}
...
def featurize(df: pd.DataFrame, mgmt_depth: pd.Series) -> pd.DataFrame:
    ...
    city = df["Company City"].fillna("").str.strip().str.lower()
    is_hub_proximate = city.isin(HUB_CITIES).astype(int)
    ...
    return pd.DataFrame({
        ...
        "is_hub_proximate": is_hub_proximate,
    })
...
def main():
    ...
    X_df = featurize(df, pd.Series(depths))
    X = X_df.values
    n_hub = int(X_df["is_hub_proximate"].sum())
    print(f"is_hub_proximate tagged: {n_hub}/{len(df)} firms")
