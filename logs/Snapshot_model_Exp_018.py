...
# exp_018 — Moat Audit: same has_moat feature as exp_016, but with the
# generic "compliance" keyword removed. The keyword set is now restricted to
# *industry-specific* verticals to test whether the exp_016 RMSE win was a
# real vertical-moat signal or an artifact of `compliance` flagging 79% of
# the dataset (which already overlaps with `recurring_kw` containing
# "compliance").
MOAT_KW = [
    "hipaa", "dental", "legal", "law firm",
    "manufacturing", "pci", "regulated",
]


def has_moat(text: str) -> int:
    if not text:
        return 0
    t = text.lower()
    for k in MOAT_KW:
...
def main():
    ...
    X = X_df.values
    n_moat = int(X_df["has_moat"].sum())
    print(f"has_moat tagged (compliance excluded): {n_moat}/{len(df)} firms")
