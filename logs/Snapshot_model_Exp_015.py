...
# exp_015 — Combined Succession Flag: union of (a) founder-presence regexes
# from exp_013 and (b) explicit acquisition substrings from exp_014, *excluding*
# the financial/growth-press signals that fired the false positive on World
# Synergy in exp_014.
SUCCESSION_RED_FLAG_REGEXES = [
...
SUCCESSION_RED_FLAG_KW = [
    "acquired by", "subsidiary of", "division of", "part of the family",
]


def is_succession_red_flag(text: str) -> int:
    if not text:
        return 0
    t = text.lower()
    for k in SUCCESSION_RED_FLAG_KW:
        if k in t:
            return 1
    for pat in SUCCESSION_RED_FLAG_REGEXES:
        if pat.search(text):
            return 1
    return 0
...
def featurize(df: pd.DataFrame, mgmt_depth: pd.Series) -> pd.DataFrame:
    ...
    succession_red_flag = sr_text.apply(is_succession_red_flag)
    ...
    return pd.DataFrame({
        ...
        "succession_red_flag": succession_red_flag,
    })
...
def main():
    ...
    X = X_df.values
    n_sr = int(X_df["succession_red_flag"].sum())
    print(f"succession_red_flag tagged: {n_sr}/{len(df)} firms")
