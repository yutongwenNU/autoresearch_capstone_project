...
def main():
    ...
    coefs = full_model.named_steps["lasso"].coef_
    feature_names = list(featurize(df, pd.Series(depths)).columns)
    coef_pairs = sorted(zip(feature_names, coefs), key=lambda x: -abs(x[1]))
    n_zero = sum(1 for _, c in coef_pairs if c == 0.0)
    print(f"Lasso coefficients (standardized) — {n_zero}/{len(coef_pairs)} pruned to zero:")
    for name, c in coef_pairs:
        marker = "  (pruned)" if c == 0.0 else ""
        print(f"  {name:>16s}: {c:+.4f}{marker}")
